...
本参赛版本新增了独立的生产力记忆协议。`/v1/search` 负责召回；`/v1/brief` 在本地对召回结果做确定性编排，输出 `answer_packet`：`summary`、`confidence`、带相对引用的 `evidence`、`uncertainties`、`conflicts`、`open_loops` 和 `next_steps`。它不调用云端大模型，也不会把不确定的笔记拼成确定答案。

协议会按核验日期标记过期证据，按项目和标题聚合同一主题的不同版本，在状态或内容不一致时返回冲突标记，并把开放事项提升为下一步。这样生产力 Agent 可以直接消费结构化结果，知道什么时候继续执行、什么时候必须停下来核对原文。这是本次参赛版本新增的 Agent 工作流能力，不是对原仓库的简单改名或文案改写。
